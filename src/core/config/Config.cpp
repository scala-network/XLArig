...
bool xlarig::Config::isHwAES() const
{
...
void xlarig::Config::setAesMode(const rapidjson::Value &aesMode)
{
    if (aesMode.IsBool()) {
...
void xlarig::Config::setAlgoVariant(int av)
{
...
void xlarig::Config::setMaxCpuUsage(int max)
{
    if (max > 0 && max <= 100) {
        m_maxCpuUsage = max;
    }
...
void xlarig::Config::setPriority(int priority)
{
...
void xlarig::Config::setThreads(const rapidjson::Value &threads)
{
    if (threads.IsArray()) {
        m_threads.cpu.clear();
...
xlarig::AlgoVariant xlarig::Config::getAlgoVariant() const
{
#   ifdef XMRIG_ALGO_CN_LITE
    if (m_algorithm.algo() == xlarig::CRYPTONIGHT_LITE) {
...
#ifdef XMRIG_ALGO_CN_LITE
xlarig::AlgoVariant xlarig::Config::getAlgoVariantLite() const
{
    if (m_algoVariant <= AV_AUTO || m_algoVariant >= AV_MAX) {
...
#ifndef XMRIG_NO_ASM
void xlarig::Config::setAssembly(const rapidjson::Value &assembly)
{
