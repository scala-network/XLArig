...
xlarig::HttpResponse::HttpResponse(uint64_t id, int statusCode) :
    m_id(id),
...
bool xlarig::HttpResponse::isAlive() const
{
...
void xlarig::HttpResponse::end(const char *data, size_t size)
{
    if (!isAlive()) {
        return;
    }
